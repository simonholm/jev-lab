...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let mut output = BufWriter::new(OpenOptions::new().write(true).create_new(true).open(root.join("experiment5-jev-results.jsonl"))?);
    ...
    for line in records.lines() {
        ...
        let http_response = match result {
            ...
            Err(error) => {
                writeln!(output, "{}", json!({"record": record, "latency_seconds": started.elapsed().as_secs_f64(), "error": error.to_string()}))?;
                output.flush()?;
        ...
        if !status.is_success() {
            writeln!(output, "{}", json!({"record": record, "latency_seconds": started.elapsed().as_secs_f64(), "http_status": status.as_u16(), "raw_body": body}))?;
            output.flush()?;
            return Err(format!("{}: OpenRouter returned HTTP {}", record.id, status).into());
        }
    ...
    println!("\nRecords: {count}");
    println!("Full run wall time: {:.3} s", started_run.elapsed().as_secs_f64());
