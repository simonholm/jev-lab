...
Run with Python 3.10 or newer; there are no third-party dependencies:

```sh
export OPENROUTER_API_KEY=... # set this privately in your shell
python3 run.py
```

The script makes one sequential OpenRouter Decisions API request per record, prints the selected category and raw returned probabilities, and writes the complete API response plus latency to ignored `results.jsonl`. It reports total input tokens, reported cost, and average request latency. A missing key stops before any request or result file is created. A failed request stops the run; results from earlier records remain in `results.jsonl`.
