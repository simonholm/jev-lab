...
Experiment 3 uses the exact same `experiment2-records.jsonl` input and model with a more explicit rubric. Its human labels were frozen in `experiment3-expected.jsonl` before the live run and are never sent to Jev. `experiment3-analysis.md` records the label changes, baseline, and comparison.
...
The Rust CLI runs Experiment 3: it makes one sequential OpenRouter Decisions API request per record, prints the selected category and Noul probabilities, and writes the complete API response (including Choice probabilities) plus latency to ignored `experiment3-results.jsonl`. It reports total input tokens, reported cost, and average request latency. A missing key stops before any request or result file is created. A failed request stops the run; results from earlier records remain in `experiment3-results.jsonl`.

This experiment does not optimize confidence thresholds.
